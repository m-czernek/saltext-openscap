...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/openscap/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/openscap/*")
            ...
            #    "--include=src/saltext/openscap/*,tests/*"
            # )
        finally:
